...
public class WaveInfoDisplay extends Table{
    ...
    public WaveInfoDisplay(){
        ...
        table(buttons -> {
            ...
            buttons.button(Icon.waves, Styles.clearNonei, iconMed, waveInfoDialog::show).tooltip(arc.Core.bundle.get("mdtx.ui.wave_info")); // 原文本:波次信息
            ...
            buttons.button("R", Styles.cleart, () -> setWaveOffset(0)).tooltip(arc.Core.bundle.get("mdtx.ui.restore_current_wave")); // 原文本:恢复当前波次
            buttons.button("J", Styles.cleart, () -> ui.showConfirm(arc.Core.bundle.get("mdtx.ui.this_is_a_cheat_feature_njump_to_the"), () -> { // 原文本:[red]这是一个作弊功能[]\n快速跳转到目标波次(不刷兵)
                state.wave += waveOffset;
                setWaveOffset(0);
            })).tooltip(arc.Core.bundle.get("mdtx.ui.force_skip_waves")).disabled((b) -> net.client()); // 原文本:强制跳波

            buttons.add().growX();
            buttons.add("♐>");
            buttons.button(Icon.wavesSmall, Styles.clearNonei, iconMed, () -> ShareFeature.shareWaveInfo(state.wave + waveOffset)).tooltip(arc.Core.bundle.get("mdtx.ui.share_wave_information")); // 原文本:分享波次信息
            buttons.button(Icon.powerSmall, Styles.clearNonei, iconMed, ShareFeature::shareTeamPower).tooltip(arc.Core.bundle.get("mdtx.ui.share_power_status")); // 原文本:分享电力情况
            buttons.button(new TextureRegionDrawable(Items.copper.uiIcon), Styles.clearNonei, iconSmall, ShareFeature::openShareItemDialog).tooltip(arc.Core.bundle.get("mdtx.ui.share_inventory_status")); // 原文本:分享库存情况
            buttons.button(Icon.unitsSmall, Styles.clearNonei, iconMed, ShareFeature::openShareUnitDialog).tooltip(arc.Core.bundle.get("mdtx.ui.share_unit_count")); // 原文本:分享单位数量
        }).fillX().row();
    ...
    private void setWaveOffsetDialog(){
        Dialog lsSet = new BaseDialog(arc.Core.bundle.get("mdtx.ui.wave_settings")); // 原文本:波次设定
        lsSet.cont.add(arc.Core.bundle.get("mdtx.ui.set_target_wave")).padRight(5f).left(); // 原文本:设定查询波次
