...
public class UnitFactoryDialog extends BaseDialog{
    ...
    public UnitFactoryDialog(){
        super(mindustryX.bundles.UiTexts.ui("unit_factory")); // 原文本:单位工厂
    ...
    private void setup(){
        ...
        buttons.button(mindustryX.bundles.UiTexts.ui("reset"), Icon.refresh, () -> { // 原文本:重置
            resetUnit(spawnUnit);
            rebuildTables();
        });
        buttons.button(mindustryX.bundles.UiTexts.ui("spawned"), Icon.modeAttack, this::spawn); // 原文本:[orange]生成！
    ...
    private void rebuild(){
        ...
        rightTable.table(Tex.pane, settings -> {
            ...
            settings.table(randDstTable -> {
                randDstTable.add(mindustryX.bundles.UiTexts.ui("spawn_range")); // 原文本:生成范围：
                randDstTable.field(Strings.autoFixed(unitRandDst, 3), text -> unitRandDst = Float.parseFloat(text)).valid(Strings::canParsePositiveFloat).tooltip(mindustryX.bundles.UiTexts.ui("randomly_generated_within_this_range_near_the_target_point")).maxTextLength(6).padLeft(4f); // 原文本:在目标点附近的这个范围内随机生成
                randDstTable.add(mindustryX.bundles.UiTexts.ui("tiles")).expandX().left(); // 原文本:格
            }).row();
    ...
    private void setupPosTable(){
        posTable.add(mindustryX.bundles.UiTexts.ui("spawn_location")); // 原文本:生成位置:
        ...
        posTable.button(Icon.pickSmall, Styles.clearNonei, () -> {
            ...
            UIExt.announce(mindustryX.bundles.UiTexts.ui("tap_the_screen_to_capture_coordinates"), 2f); // 原文本:[green]点击屏幕采集坐标
        });
        ...
        posTable.button(Icon.eyeSmall, Styles.clearNonei, () -> {
            ...
            UIExt.announce(mindustryX.bundles.UiTexts.ui("click_the_screen_to_return"), 2f); // 原文本:[green]点击屏幕返回
        }).padLeft(4);
    ...
    private void setupCountTable(){
        countTable.add(mindustryX.bundles.UiTexts.ui("generate_quantity")); // 原文本:生成数量:
            ...
            rightTable.table(Tex.pane, labelTable -> {
                ...
                labelTable.table(null, buttons -> {
                    buttons.button(Icon.copySmall, Styles.clearNonei, 16, () -> {
                        ...
                        ui.announce(mindustryX.bundles.UiTexts.ui("copied_successfully") + label.getText(), 3); // 原文本:复制成功:
                    }).size(28);
    ...
    private void rebuildPropertiesTable(Unit unit, Table propertiesTable){
        ...
        propertiesTable.table(t -> t.check(mindustryX.bundles.UiTexts.ui("flight_mode"), unit.elevation > 0, a -> unit.elevation = a ? 1 : 0).padBottom(5f).padRight(10f).tooltip(mindustryX.bundles.UiTexts.ui("spawns_units_that_fly"), true) // 原文本:飞行模式 | [orange]生成的单位会飞起来
        .checked(b -> unit.elevation > 0));

        propertiesTable.row();

        propertiesTable.table(healthTable -> {
            healthTable.add(mindustryX.bundles.UiTexts.ui("health_label")); // 原文本:[red]血量：
        ...
        propertiesTable.table(shieldTable -> {
            shieldTable.add(mindustryX.bundles.UiTexts.ui("shield")); // 原文本:[yellow]护盾：
    ...
    private void rebuildItemTable(Unit unit, Table itemTable){
        ...
        itemTable.add(mindustryX.bundles.UiTexts.ui("carried_item")); // 原文本:携带物品:
    ...
    private void rebuildTeamTable(Unit unit, Table teamTable){
        ...
        teamTable.add(mindustryX.bundles.UiTexts.ui("spawn_team")); // 原文本:生成队伍:
        teamTable.label(() -> {
            Team team = unit.team;
        ...
        teamTable.add(mindustryX.bundles.UiTexts.ui("team_id")).padLeft(4); // 原文本:队伍ID:
    ...
    private void rebuildEffectsTable(Unit unit, Table effectTable){
        ...
        Runnable rebuildInfo = () -> {
            ...
            effectInfo.add(mindustryX.bundles.UiTexts.ui("damage")); // 原文本:[red]伤害
            effectInfo.add(FormatDefault.format(status[0])).expandX().right();

            effectInfo.add(mindustryX.bundles.UiTexts.ui("hp")).padLeft(12f); // 原文本:[acid]血量
            effectInfo.add(FormatDefault.format(status[1])).expandX().right();
            effectInfo.row();

            effectInfo.add(mindustryX.bundles.UiTexts.ui("movement_speed")); // 原文本:[cyan]移速
            effectInfo.add(FormatDefault.format(status[2])).expandX().right();

            effectInfo.add(mindustryX.bundles.UiTexts.ui("attack_speed")).padLeft(12f); // 原文本:[violet]攻速
            effectInfo.add(FormatDefault.format(status[3])).expandX().right();
            effectInfo.row();

            effectInfo.add(mindustryX.bundles.UiTexts.ui("build_speed")); // 原文本:[accent]建速
            effectInfo.add(FormatDefault.format(status[4])).expandX().right();

            effectInfo.add(mindustryX.bundles.UiTexts.ui("resistance")).padLeft(12f); // 原文本:[purple]阻力
            effectInfo.add(FormatDefault.format(status[5])).expandX().right();
            effectInfo.row();

            if(status[6] >= 0){
                effectInfo.add(mindustryX.bundles.UiTexts.ui("armor")); // 原文本:[teal]装甲
        ...
        unitStatus.each(entry -> {
            ...
            table.table(Tex.whiteui, t -> {
                ...
                if(entry.effect.permanent){
                    t.add(mindustryX.bundles.UiTexts.ui("permanent_status")).expandX(); // 原文本:<永久状态>
                }else if(entry.effect.reactive){
                    t.add(mindustryX.bundles.UiTexts.ui("temporary_status")).expandX(); // 原文本:<瞬间状态>
                }else{
                    t.table(bottom -> {
                        ...
                        bottom.add(mindustryX.bundles.UiTexts.ui("sec")); // 原文本:秒
            ...
            if(effect.dynamic) table.add(new Card(Pal.darkestGray, Card.grayOuterDark, t -> {
                ...
                t.add(mindustryX.bundles.UiTexts.ui("damage")).style(Styles.outlineLabel); // 原文本:[red]伤害
                ...
                t.add(mindustryX.bundles.UiTexts.ui("hp")).style(Styles.outlineLabel); // 原文本:[acid]血量
                ...
                t.add(mindustryX.bundles.UiTexts.ui("movement_speed")).style(Styles.outlineLabel); // 原文本:[cyan]移速
                ...
                t.add(mindustryX.bundles.UiTexts.ui("attack_speed")).style(Styles.outlineLabel); // 原文本:[violet]攻速
                ...
                t.add(mindustryX.bundles.UiTexts.ui("build_speed")).style(Styles.outlineLabel); // 原文本:[accent]建速
                ...
                t.add(mindustryX.bundles.UiTexts.ui("resistance")).style(Styles.outlineLabel); // 原文本:[purple]阻力
                ...
                t.add(mindustryX.bundles.UiTexts.ui("armor")).style(Styles.outlineLabel); // 原文本:[teal]装甲
    ...
    private void rebuildPayloadTable(Unit unit, Table payloadTable){
        ...
        payloadTable.table(Tex.whiteui, table -> {
            ...
            table.table(Styles.grayPanel, buttons -> {
                ...
                buttons.button(mindustryX.bundles.UiTexts.ui("load_building"), new TextureRegionDrawable(Blocks.siliconSmelter.uiIcon), Styles.flatt, 32, // 原文本:装载建筑
                ...
                buttons.button(mindustryX.bundles.UiTexts.ui("load_unit"), new TextureRegionDrawable(UnitTypes.alpha.uiIcon), Styles.flatt, 32f, // 原文本:装载单位
                ...
                buttons.button(mindustryX.bundles.UiTexts.ui("load_self"), Icon.add, Styles.flatt, () -> { // 原文本:装载自己
    ...
    private void simpleFactory(Unit unit){
        BaseDialog dialog = new BaseDialog(mindustryX.bundles.UiTexts.ui("unit_factory")); // 原文本:单位工厂
        ...
        dialog.buttons.button(mindustryX.bundles.UiTexts.ui("reset"), Icon.refresh, () -> { // 原文本:重置
            resetUnit(unit);
            rebuildTables();
        });
    ...
    private static class UnitStack{
        ...
        public String name(){
            return mod == null ? Core.bundle.get("vanilla", mindustryX.bundles.UiTexts.ui("vanilla")) : mod.meta.displayName; // 原文本:原版
        }
