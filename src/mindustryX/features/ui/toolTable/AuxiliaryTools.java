...
public class AuxiliaryTools extends Table{
    ...
    protected void rebuild(){
        defaults().size(40);
        aiButton(new ArcMinerAI(), UnitTypes.mono.region, mindustryX.bundles.UiTexts.ui("miner_ai")); // 原文本:矿机AI
        aiButton(new BuilderAI(), UnitTypes.poly.region, mindustryX.bundles.UiTexts.ui("builder_ai")); // 原文本:重建AI
        aiButton(new RepairAI(), UnitTypes.mega.region, mindustryX.bundles.UiTexts.ui("repair_ai")); // 原文本:修复AI
        aiButton(new DefenderAI(), UnitTypes.oct.region, mindustryX.bundles.UiTexts.ui("defender_ai")); // 原文本:保护AI
        ...
        }).tooltip(mindustryX.bundles.UiTexts.ui("add_destroyed_buildings_to_build_queue")); // 原文本:在建造列表加入被摧毁建筑
        var t = button(new TextureRegionDrawable(Items.copper.uiIcon), Styles.clearNoneTogglei, () -> AutoFill.enable ^= true).tooltip(mindustryX.bundles.UiTexts.ui("auto_fill")).checked((b) -> AutoFill.enable).get(); // 原文本:一键装填
        SettingsV2.bindQuickSettings(t, AutoFill.INSTANCE.getSettings());
        toggleButton(Icon.modeAttack, "autotarget", mindustryX.bundles.UiTexts.ui("auto_attack")); // 原文本:自动攻击
        toggleButton(new TextureRegionDrawable(UnitTypes.vela.uiIcon), "forceBoost", mindustryX.bundles.UiTexts.ui("force_boost")); // 原文本:强制助推
        toggleButton(Icon.eyeSmall, "detach-camera", mindustryX.bundles.UiTexts.ui("detached_camera")); // 原文本:视角脱离玩家

        if(!mobile) return;
        row();
        toggleButton(Icon.unitsSmall, mindustryX.bundles.UiTexts.ui("command_mode"), () -> control.input.commandMode = !control.input.commandMode).checked(b -> control.input.commandMode); // 原文本:指挥模式
        toggleButton(Icon.pause, mindustryX.bundles.UiTexts.ui("construction_suspended"), () -> control.input.isBuilding = !control.input.isBuilding).checked(b -> control.input.isBuilding); // 原文本:暂停建造
        scriptButton(Icon.up, mindustryX.bundles.UiTexts.ui("pick_up_payload"), () -> control.input.tryPickupPayload()); // 原文本:捡起载荷
        scriptButton(Icon.down, mindustryX.bundles.UiTexts.ui("drop_payload"), () -> control.input.tryDropPayload()); // 原文本:丢下载荷
        scriptButton(new TextureRegionDrawable(Blocks.payloadConveyor.uiIcon), mindustryX.bundles.UiTexts.ui("enter_the_conveyor_belt"), () -> { // 原文本:进入传送带
    ...
    private void showAiSettingDialog(){
        ...
        BaseDialog dialog = new BaseDialog(mindustryX.bundles.UiTexts.ui("arc_ai_configurator")); // 原文本:ARC-AI设定器

        dialog.cont.table(t -> {
            t.add(mindustryX.bundles.UiTexts.ui("miner_ai_ore_filter")).color(Pal.accent).pad(cols / 2f).center().row(); // 原文本:minerAI-矿物筛选器
