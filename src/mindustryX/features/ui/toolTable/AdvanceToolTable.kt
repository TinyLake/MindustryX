...
class AdvanceToolTable : Table() {
    ...
    init {
        ...
        row().add(mindustryX.bundles.ui("single_player_map_tools_only")).color(Color.yellow).colspan(2) // 原文本:警告：该页功能主要供单机作图使用

        row().add(mindustryX.bundles.ui("unit")) // 原文本:单位：
        with(table().growX().get()) {
            ...
            }.tooltip(mindustryX.bundles.ui("fill_core_resources")) // 原文本:填满核心的所有资源
            button(Items.copper.emoji() + "[red]-", Styles.cleart) {
                val core = Vars.player.core() ?: return@button
                core.items.clear()
            }.tooltip(mindustryX.bundles.ui("clear_all_core_resources")) // 原文本:清空核心的所有资源
            button(UnitTypes.gamma.emoji() + "+", Styles.cleart) {
                ...
                cloneUnit.add()
            }.tooltip(mindustryX.bundles.ui("clone")) // 原文本:克隆
            button(UnitTypes.gamma.emoji() + "[red]×", Styles.cleart) { if (!Vars.player.dead()) Vars.player.unit().kill() }.tooltip(mindustryX.bundles.ui("self_destruct")) // 原文本:自杀
            button(Icon.waves, Styles.clearNonei) { factoryDialog.show() }.tooltip(mindustryX.bundles.ui("unit_factory_x")) // 原文本:单位工厂-X
        }

        row().add(mindustryX.bundles.ui("team")) // 原文本:队伍：
        with(table().growX().get()) {
            defaults().pad(4f)
            var count = 0
            ...
            button("+", Styles.flatToggleMenut) { UIExt.teamSelect.pickOne({ team: Team? -> Vars.player.team(team) }, Vars.player.team()) }
                ...
                .tooltip(mindustryX.bundles.ui("more_teams")) // 原文本:更多队伍选择
        }

        row().add(mindustryX.bundles.ui("buildings")) // 原文本:建筑：
        with(table().growX().get()) {
            defaults().pad(4f)
            button(mindustryX.bundles.ui("god_mode"), Styles.flatToggleMenut) { LogicExt.worldCreator0.toggle() } // 原文本:创世神
                .minWidth(120f).height(Vars.iconMed)
                .checked { LogicExt.worldCreator }.wrapLabel(true)
            button(mindustryX.bundles.ui("unlock"), Styles.flatToggleMenut) { // 原文本:解禁
                VarsX.allUnlocked.toggle()
            }.minWidth(120f).height(Vars.iconMed).checked { VarsX.allUnlocked.value }.tooltip(mindustryX.bundles.ui("unlock_and_allow_all_blocks")).wrapLabel(true).row() // 原文本:显示并允许建造所有物品
            button(mindustryX.bundles.ui("terrain_blueprint"), Styles.flatToggleMenut) { LogicExt.terrainSchematic0.toggle() } // 原文本:地形蓝图
                .minWidth(120f).height(Vars.iconMed)
                .checked { LogicExt.terrainSchematic }.wrapLabel(true)
            button(mindustryX.bundles.ui("instant"), Styles.cleart) { // 原文本:瞬间完成
                Vars.player.unit()?.apply {
                    if (!canBuild()) {
                        UIExt.announce(mindustryX.bundles.ui("current_unit_cannot_build")) // 原文本:[red]当前单位不可建筑
                        return@apply
                    }
        ...
        row().add(mindustryX.bundles.ui("rules")) // 原文本:规则：
        with(table().growX().get()) {
            defaults().pad(4f)
            button(Iconc.map.toString(), Styles.cleart) { mapInfoDialog.show() }.width(40f)
            button(mindustryX.bundles.ui("cheat"), Styles.flatToggleMenut) { Vars.player.team().rules().cheat = !Vars.player.team().rules().cheat } // 原文本:无限火力
                .checked { Vars.player.team().rules().cheat }.tooltip(mindustryX.bundles.ui("toggle_your_team_s_cheat")).wrapLabel(false) // 原文本:开关自己队的无限火力
            button(mindustryX.bundles.ui("editor"), Styles.flatToggleMenut) { Vars.state.rules.editor = !Vars.state.rules.editor } // 原文本:编辑器
                .checked { Vars.state.rules.editor }.wrapLabel(false)
            button(mindustryX.bundles.ui("sandbox"), Styles.flatToggleMenut) { Vars.state.rules.infiniteResources = !Vars.state.rules.infiniteResources } // 原文本:沙盒
        ...
        row().add(mindustryX.bundles.ui("hourglass")) // 原文本:沙漏：
