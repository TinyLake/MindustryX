...
public class AdvanceBuildTool extends Table{
    ...
    public AdvanceBuildTool(){
        ...
        Events.run(Trigger.draw, () -> {
            if(placement == BuildRange.zone){
                ...
                Draw.color();
                FuncX.drawText(selection.getCenter(Tmp.v1).scl(tilesize), i("ui.simple.build-area"), Scl.scl(1.25f), Color.white);
            }
    ...
    protected void rebuild(){
        ...
        add().height(40);
        button("", Styles.clearTogglet, () -> placement = BuildRange.global).checked((b) -> placement == BuildRange.global).tooltip(i("ui.simple.global-range")).size(30f);
        button("\uE818", Styles.clearTogglet, () -> {
            selection = control.input.lastSelection;
            if(selection.area() < 10f){
                UIExt.announce(i("ui.simple.the-currently-selected-area-is-empty-please-use"));
                return;
            }
            placement = BuildRange.zone;
        }).checked((b) -> placement == BuildRange.zone).tooltip(i("ui.simple.selection-range")).size(30f);
        button(Blocks.coreShard.emoji(), Styles.clearTogglet, () -> {
            placement = BuildRange.team;
            rebuild();
        }).checked((b) -> placement == BuildRange.team).tooltip(i("ui.simple.team-range")).size(30f);
        button(UnitTypes.gamma.emoji(), Styles.clearTogglet, () -> placement = BuildRange.player).checked((b) -> placement == BuildRange.player).tooltip(i("ui.simple.player-build-range")).size(30f);
        ...
        }).height(30f).tooltip(i("ui.simple.find-blocks")).wrapLabel(false).get();
        findButton.clicked(() -> {
        ...
        findButton.button(Icon.settingsSmall, Styles.clearTogglei, iconSmall, () -> {
            if(target == null){
                UIExt.announce(i("ui.simple.yellow-current-selection-is-empty-select-a-block"));
                return;
            }
            find = target;
            searchIndex = 0;
            placement = BuildRange.find;
            rebuild();
        }).tooltip(i("ui.simple.set-target")).padRight(2f);
