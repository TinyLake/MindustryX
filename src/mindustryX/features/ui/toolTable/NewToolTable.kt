...
object NewToolTable : Table() {
    ...
    init {
        ...
        button(mindustryX.bundles.UiTexts.ui("shortcut_message_center"), mindustryX.bundles.UiTexts.ui("message_center")) { UIExt.arcMessageDialog.show() } // 原文本:信 | 中央监控室
        button("S", mindustryX.bundles.UiTexts.ui("sync_a_wave")) { Call.sendChatMessage("/sync") } // 原文本:同步一波
        button(mindustryX.bundles.UiTexts.ui("shortcut_observer_mode"), mindustryX.bundles.UiTexts.ui("observer_mode")) { Call.sendChatMessage("/ob") } // 原文本:观 | 观察者模式
        button(mindustryX.bundles.UiTexts.ui("shortcut_server_info"), mindustryX.bundles.UiTexts.ui("server_info_build")) { Call.sendChatMessage("/broad") } // 原文本:版 | 服务器信息版
        toggle(mindustryX.bundles.UiTexts.ui("shortcut_fog"), mindustryX.bundles.UiTexts.ui("fog_of_war"), { Vars.state.rules.fog }) { // 原文本:雾 | 战争迷雾
            Vars.state.rules.fog = Vars.state.rules.fog xor true
        }.disabled { Vars.state.rules.pvp && Vars.player.team().id != 255 }
        button(mindustryX.bundles.UiTexts.ui("shortcut_surrender_vote"), mindustryX.bundles.UiTexts.ui("surrender_vote")) { // 原文本:[white]法 | 法国军礼
            Vars.ui.showConfirm(mindustryX.bundles.UiTexts.ui("surrender_vote_confirm")) { Call.sendChatMessage("/vote gameover") } // 原文本:受不了，直接投降？
        }
        toggle(mindustryX.bundles.UiTexts.ui("shortcut_scan_mode"), mindustryX.bundles.UiTexts.ui("scan_mode"), { RenderExt.transportScan.value }) { RenderExt.transportScan.toggle() } // 原文本:扫 | 扫描模式

        toggle(mindustryX.bundles.UiTexts.ui("shortcut_block_render"), mindustryX.bundles.UiTexts.ui("block_rendering"), { RenderExt.blockRenderLevel > 0 }) { RenderExt.blockRenderLevel0.cycle() } // 原文本:块 | 建筑显示
        toggle(mindustryX.bundles.UiTexts.ui("shortcut_unit_render"), mindustryX.bundles.UiTexts.ui("unit_rendering"), { !RenderExt.unitHide.value }) { RenderExt.unitHide.toggle() } // 原文本:兵 | 兵种显示
        toggle(mindustryX.bundles.UiTexts.ui("shortcut_bullet_render"), mindustryX.bundles.UiTexts.ui("bullet_rendering"), { !RenderExt.noBulletShow.value }) { RenderExt.noBulletShow.toggle() } // 原文本:弹 | 子弹显示
        toggle(mindustryX.bundles.UiTexts.ui("fx"), mindustryX.bundles.UiTexts.ui("effects_rendering"), { Vars.renderer.enableEffects }) { Settings.toggle("effects") } // 原文本:效 | 特效显示
        toggle(mindustryX.bundles.UiTexts.ui("shortcut_wall_shadow"), mindustryX.bundles.UiTexts.ui("wall_shadow_rendering"), { Vars.enableDarkness }) { Vars.enableDarkness = !Vars.enableDarkness } // 原文本:墙 | 墙体阴影显示
        toggle("${Iconc.map}", mindustryX.bundles.UiTexts.ui("minimap"), { Core.settings.getBool("minimap") }) { Settings.toggle("minimap") } // 原文本:小地图显示
        toggle(mindustryX.bundles.UiTexts.ui("shortcut_hitbox"), mindustryX.bundles.UiTexts.ui("hitbox_overlay"), { RenderExt.unitHitbox.value }) { RenderExt.unitHitbox.toggle() } // 原文本:箱 | 碰撞箱显示

        button("${Iconc.blockRadar}", mindustryX.bundles.UiTexts.ui("radar_toggle")) { ArcRadar.mobileRadar = !ArcRadar.mobileRadar }.get().also { // 原文本:雷达开关
            SettingsV2.bindQuickSettings(it, ArcRadar.settings)
        }
        toggle("${Iconc.blockWorldProcessor}", mindustryX.bundles.UiTexts.ui("remove_logic_lock"), { Core.settings.getBool("removeLogicLock") }) { // 原文本:移除逻辑锁定
            Settings.toggle("removeLogicLock")
            if (Core.settings.getBool("removeLogicLock")) {
                Vars.control.input.logicCutscene = false
                Vars.ui.announce(mindustryX.bundles.UiTexts.ui("logic_camera_lock_removed")) // 原文本:已移除逻辑视角锁定
            }
        }
        toggle(Blocks.worldMessage.emoji(), mindustryX.bundles.UiTexts.ui("show_all_message_blocks"), { RenderExt.displayAllMessage }) { Settings.toggle("displayallmessage") } // 原文本:信息板全显示
        button("${Iconc.itemCopper}", mindustryX.bundles.UiTexts.ui("ore_info")) { floorStatisticDialog() } // 原文本:矿物信息

        button("${Iconc.fill}", mindustryX.bundles.UiTexts.ui("effects_library")) { EffectsDialog.withAllEffects().show() } // 原文本:特效大全
        button("${Iconc.star}", mindustryX.bundles.UiTexts.ui("ui_toolkit")) { uiTableDialog().show() } // 原文本:ui大全
    ...
    data class CustomButton(val name: String, val content: String) {
        constructor() : this("?", mindustryX.bundles.UiTexts.ui("no_command_entered")) // 原文本:未输入指令
        ...
        override fun buildUI() = Table().let { table ->
            ...
            table.collapser(Table().apply {
                ...
                update {
                    ...
                    add(mindustryX.bundles.UiTexts.ui("index")); add(mindustryX.bundles.UiTexts.ui("display_name")); add(mindustryX.bundles.UiTexts.ui("message_arg_js_starts_with_script")); row() // 原文本:序号 | 显示名 | 消息(@js 开头为脚本)
                    ...
                    add(mindustryX.bundles.UiTexts.ui("before_adding_new_instructions_please_save_the_edited_instructions_first")).colspan(columns).center().padTop(-4f).row() // 原文本:[yellow]添加新指令前，请先保存编辑的指令
                }
            }) { shown }.growX()
            table.row()
        }
    ...
    private fun floorStatisticDialog() {
        val dialog = BaseDialog(mindustryX.bundles.UiTexts.ui("arc_ore_statistics")) // 原文本:ARC-矿物统计
        val table = dialog.cont
        table.clear()

        table.table { c: Table ->
            c.add(mindustryX.bundles.UiTexts.ui("ore_count_surface_wall")).color(Pal.accent).center().fillX().row() // 原文本:矿物矿(地表/墙矿)
            ...
            c.add(mindustryX.bundles.UiTexts.ui("liquids")).color(Pal.accent).center().fillX().row() // 原文本:液体
            c.image().color(Pal.accent).fillX().row()
            c.table { list: Table ->
                var i = 0
    ...
    private fun uiTableDialog() = BaseDialog(mindustryX.bundles.UiTexts.ui("ui_icon_library")).apply { // 原文本:UI图标大全
        ...
        cont.table().growX().get().apply {
            ...
            row()
            add(mindustryX.bundles.UiTexts.ui("staging_area")).color(Pal.lightishGray).padRight(16f) // 原文本:暂存区
        ...
        Table().apply {
            ...
            add(mindustryX.bundles.UiTexts.ui("color")).color(Pal.accent).center().row() // 原文本:颜色
            ...
            add(mindustryX.bundles.UiTexts.ui("item")).color(Pal.accent).center().row() // 原文本:物品
            image().color(Pal.accent).fillX().row()
            GridTable().apply {
            ...
            add(mindustryX.bundles.UiTexts.ui("icon")).color(Pal.accent).center().row() // 原文本:图标
            image().color(Pal.accent).fillX().row()
            GridTable().apply {
                defaults().size(Vars.iconLarge)
