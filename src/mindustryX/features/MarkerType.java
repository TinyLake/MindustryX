...
public class MarkerType{
    ...
    private static final Element markHitter = new Table(){{
        ...
        center().add(mindustryX.bundles.FuncX.ui("mark_mode_tap_the_screen_to_place_a_mark"), Styles.outlineLabel); // 原文本:[cyan]标记模式,点击屏幕标记.
        tapped(() -> {
    ...
    public void markWithMessage(Vec2 pos){
        if(last != null && last.time < heatTime){
            Vars.ui.announce(mindustryX.bundles.FuncX.ui("please_don_t_tag_too_often")); // 原文本:请不要频繁标记!
            return;
        }
        last = at(pos);
    ...
    public static void newMarkFromChat(String text, Vec2 pos){
        ...
        if(text.contains(mindustryX.bundles.FuncX.ui("rally"))){ // 原文本:集合
            type = gatherMark;
        }
