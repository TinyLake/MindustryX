...
public class MarkerType{
    ...
    private static final Element markHitter = new Table(){{
        ...
        center().add(i("ui.simple.cyan-mark-mode-tap-the-screen-to-place-a-mark"), Styles.outlineLabel);
        tapped(() -> {
    ...
    public void markWithMessage(Vec2 pos){
        if(last != null && last.time < heatTime){
            Vars.ui.announce(i("ui.simple.please-don-t-tag-too-often"));
            return;
        }
        last = at(pos);
    ...
    public static void newMarkFromChat(String text, Vec2 pos){
        ...
        if(text.contains(i("marker.Gather.name"))){
            type = gatherMark;
        }
