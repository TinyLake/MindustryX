...
object ShareFeature {
    ...
    private fun uploadPasteBin(content: String, callback: (String?) -> Unit) {
        ...
        req.error {
            Core.app.post {
                Vars.ui.showException(mindustryX.bundles.UiTexts.bundle().i("上传失败，再重试一下？"), it) // 原文本:上传失败，再重试一下？
    ...
    fun shareSchematicClipboard(schem: Schematic) {
        uploadPasteBin(Vars.schematics.writeBase64(schem)) { link: String? ->
            val msg = buildString {
                ...
                append(mindustryX.bundles.UiTexts.bundle().i("蓝图名：")).append(schem.name()).append("\n") // 原文本:蓝图名：
                append(mindustryX.bundles.UiTexts.bundle().i("分享者：")).append(Vars.player.name).append("\n") // 原文本:分享者：
                append(mindustryX.bundles.UiTexts.bundle().i("蓝图造价：")) // 原文本:蓝图造价：
                val arr = schem.requirements()
                for (stack in arr) {
                    append(stack.item.emoji()).append(stack.item.localizedName).append(stack.amount).append("|")
                }
                append("\n")
                append(mindustryX.bundles.UiTexts.bundle().i("电力：")) // 原文本:电力：
                ...
                append("\n")
                append(mindustryX.bundles.UiTexts.bundle().i("蓝图代码链接：")).append(link ?: "x").append("\n") // 原文本:蓝图代码链接：
                if (Vars.schematics.writeBase64(schem).length > 3500) append(mindustryX.bundles.UiTexts.bundle().i("蓝图代码过长，请点击链接查看")) // 原文本:蓝图代码过长，请点击链接查看
                else append(mindustryX.bundles.UiTexts.bundle().i("蓝图代码：\n")).append(Vars.schematics.writeBase64(schem)) // 原文本:蓝图代码：\n
            }

            Core.app.setClipboardText(Strings.stripColors(msg))
            UIExt.announce(mindustryX.bundles.UiTexts.bundle().i("已保存至剪贴板")) // 原文本:已保存至剪贴板
        }
    }
    ...
    fun newShareTable() = Table(Styles.black3).apply {
        ...
        button("T", underlineToggleT) { Vars.ui.chatfrag.nextMode() }
            .checked { _ -> Vars.ui.chatfrag.mode == ChatFragment.ChatMode.team }.tooltip(mindustryX.bundles.UiTexts.bundle().i("前缀添加/t")) // 原文本:前缀添加/t
        button(Icon.zoomSmall, Styles.clearNonei) { MarkerType.lockOnLastMark() }
            .tooltip(mindustryX.bundles.UiTexts.bundle().i("锁定上个标记点")) // 原文本:锁定上个标记点

        add("♐>").padRight(18f)
        button(Icon.mapSmall, Styles.clearNonei, Vars.iconMed) { MarkerType.toggleMarkHitterUI() }.tooltip(mindustryX.bundles.UiTexts.bundle().i("标记地图位置")) // 原文本:标记地图位置
        button(Icon.wavesSmall, Styles.clearNonei, Vars.iconMed) { shareWaveInfo(Vars.state.wave) }.tooltip(mindustryX.bundles.UiTexts.bundle().i("分享波次信息")) // 原文本:分享波次信息
        button(Icon.powerSmall, Styles.clearNonei, Vars.iconMed) { shareTeamPower() }.tooltip(mindustryX.bundles.UiTexts.bundle().i("分享电力情况")) // 原文本:分享电力情况
        button(TextureRegionDrawable(Items.copper.uiIcon), Styles.clearNonei, Vars.iconSmall) { openShareItemDialog() }.tooltip(mindustryX.bundles.UiTexts.bundle().i("分享库存情况")) // 原文本:分享库存情况
        button(Icon.unitsSmall, Styles.clearNonei, Vars.iconMed) { openShareUnitDialog() }.tooltip(mindustryX.bundles.UiTexts.bundle().i("分享单位数量")) // 原文本:分享单位数量
    }
    ...
    private fun resolveAt(message: String, sender: Player?): Boolean {
        ...
        if (message.contains(Vars.player.name)) {
            ...
            else Vars.ui.announce(mindustryX.bundles.UiTexts.bundle().i("[orange]你被戳了一下，请注意查看信息框哦~"), 10f) // 原文本:[orange]你被戳了一下，请注意查看信息框哦~
        }
